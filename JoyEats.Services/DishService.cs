...
namespace JoyEats.Services
{
    public class DishService : BaseService, IDishService
    {
        ...
        {
            var queryable = _dishRepository.GetQueryable();
            if (!string.IsNullOrWhiteSpace(dishPageQueryDTO.Name))
            {
                queryable = queryable.Where(d => d.Name.Contains(dishPageQueryDTO.Name));
            }
            if (dishPageQueryDTO.CategoryId.HasValue && dishPageQueryDTO.CategoryId.Value > 0 )
            {
                queryable = queryable.Where(d => d.CategoryId == dishPageQueryDTO.CategoryId.Value);
            }
            if (dishPageQueryDTO.Status.HasValue)
            {
                queryable = queryable.Where(d => d.Status == dishPageQueryDTO.Status);
            }

            var dishVoList = await queryable
